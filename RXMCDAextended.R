      ...
      if (length(elements)>0){
        for (j in 1:length(elements)){
          ...
          if(x){
            ...
            message(paste0(j, cIDs))
            if(is.element(cIDs, criteriaIDs)){
              preferenceDirection <- getNodeSet(elements[[i]], "scale/quantitative")
              out[cIDs, "preferenceDirection"] <- XML::xmlValue(preferenceDirection[[1]])
              
              ind <-getNodeSet(elements[[i]], "thresholds/threshold[@id='ind']/linear/slope")
              ind_a <- XML::xmlValue(ind[[1]])
              out[cIDs, "ind_a"] <- sapply(parse(text=ind_a), eval) 
              
              ind <-getNodeSet(elements[[i]], "thresholds/threshold[@id='ind']/linear/intercept")
              ind_b <- XML::xmlValue(ind[[1]])
              out[cIDs, "ind_b"]  <- sapply(parse(text=ind_a), eval) 
              
              pref <-getNodeSet(elements[[i]], "thresholds/threshold[@id='pref']/linear/slope")
              pref_a <- XML::xmlValue(ind[[1]])
              out[cIDs, "pref_a"]  <- sapply(parse(text=pref_a), eval) 
              
              pref <-getNodeSet(elements[[i]], "thresholds/threshold[@id='pref']/linear/intercept")
              pref_b <- XML::xmlValue(ind[[1]])
              out[cIDs, "pref_b"]  <- sapply(parse(text=pref_a), eval) 
              
              veto <-getNodeSet(elements[[i]], "thresholds/threshold[@id='veto']/linear/slope")
              veto_a <- XML::xmlValue(ind[[1]])
